...
- Prefer arrays over pointers as variables and function parameters. Use pointer only when there is no other option but using pointers. Use array indices syntax for iterating over arrays. Focus on readability more here.
  - Exceptiions:
    - If any argument is more likely to be used as destination, prefer pointer.
