 ...
 *
 * The timestamp is normalized to whole days (midnight UTC) before
 * formatting. The result is truncated when the buffer is too small.
 *
 ...
 *
 * The returned time_t is normalized to midnight UTC. The string must
...
/**
 * Returns the current system date normalized to midnight UTC.
 *
 ...
 *
 * The result is positive when later is after earlier. Both timestamps
 * are normalized to midnight before the calculation.
 *
 * @param earlier the earlier date
 * @param later the later date
 * @return the number of days between them, or 0 on invalid input
 */
