...
/**
 * Verifies that input_read_string reads lines stripped of the newline.
 */
static void test_read_string_plain_lines(void) {
    char buffer[64];

    redirect_stdin_with_content("hello world\nsecond\n");
    assert(input_read_string(buffer, sizeof buffer) == true);
    assert(strcmp(buffer, "hello world") == 0);
    assert(input_read_string(buffer, sizeof buffer) == true);
    assert(strcmp(buffer, "second") == 0);
    assert(input_read_string(buffer, sizeof buffer) == false);
}

/**
 * Verifies that input_read_string tolerates CRLF line endings.
 */
...
/**
 * Verifies that input_read_string rejects invalid arguments.
 */
static void test_read_string_invalid_arguments(void) {
    char buffer[64];

    redirect_stdin_with_content("anything\n");
    assert(input_read_string(NULL, sizeof buffer) == false);
    assert(input_read_string(buffer, 1) == false);
}

/**
 * Verifies that input_read_unsigned accepts digits up to unsigned int max.
 */
static void test_read_unsigned_valid(void) {
    unsigned int value;

    redirect_stdin_with_content("42\n0\n4294967295\n");
    assert(input_read_unsigned(&value) == true);
    assert(value == 42u);
    assert(input_read_unsigned(&value) == true);
    assert(value == 0u);
    assert(input_read_unsigned(&value) == true);
    assert(value == UINT_MAX);
    assert(input_read_unsigned(&value) == false);
}

/**
 * Verifies that input_read_unsigned rejects signs, non-digits, and overflow.
 */
static void test_read_unsigned_rejects_bad(void) {
    ...
    assert(input_read_unsigned(NULL) == false);
}
...
/**
 * Verifies that input_read_unsigned_long accepts digits up to the max value.
 */
static void test_read_unsigned_long_valid(void) {
    unsigned long int value;

    redirect_stdin_with_content("123\n18446744073709551615\n");
    assert(input_read_unsigned_long(&value) == true);
    assert(value == 123ul);
    assert(input_read_unsigned_long(&value) == true);
    assert(value == ULONG_MAX);
    assert(input_read_unsigned_long(&value) == false);
}

/**
 * Verifies that input_read_unsigned_long rejects signs, non-digits, overflow.
 */
static void test_read_unsigned_long_rejects_bad(void) {
    ...
    assert(input_read_unsigned_long(NULL) == false);
}
